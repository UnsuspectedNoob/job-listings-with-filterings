...
filters.forEach((filter) => {
  newArray = newArray
    .map((value) => {
      return {
        ...value,
        filters: [value.role, value.level, ...value.languages, ...value.tools],
      };
    })
    .filter((value) => value.filters.includes(filter));
});

console.log(
  newArray.map((value) => {
    return { company: value.company, filters: value.filters };
  })
);
